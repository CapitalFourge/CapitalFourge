...
def get_news(symbol: str, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for item in news[:limit]:
            ...
            processed["sentiment_score"] = _calculate_sentiment(
                processed["headline"] + " " + processed["summary"]
            )
    ...
    try:
        ...
        for item in news[:limit]:
            processed.append({
                ...
                "image": item.get("image", ""),
                "category": item.get("category", ""),
                "sentiment_score": _calculate_sentiment(
                    item.get("headline", "") + " " + item.get("summary", "")
                ),
                "sentiment_label": _sentiment_label(
                    _calculate_sentiment(item.get("headline", "") + " " + item.get("summary", ""))
                ),
            })
